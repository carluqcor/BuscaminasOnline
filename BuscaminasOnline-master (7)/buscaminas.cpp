...
int Buscaminas::getX(char x){
	switch (x){
		...
		default:
			std::cout<<"-Err. letra no reconocida, escribala en mayúscula en el rango de A a J\n\n";
			return -1;
			break;
	}
}
...
bool Buscaminas::ganar(){
	if( (getEncontradas()==10) || (getEncontradas2()==10) ){
		return true;
	}
	return false;
}
...
int Buscaminas::getNumero(int y){
	if(y>=0 && y<10)
		...
		std::cout<<"-Err. elija un número en el rango de 0 a 9\n\n";
		return -1;
	}
}
